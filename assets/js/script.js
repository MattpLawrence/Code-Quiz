...
function checkNameLength() {
  ...
  if (nameInput.val().length < 3) {
    //check to see if string entered is at least 3 char length
    if (questionEl.children().length < 4) {
      //check to see if alert has already been issued.
      alertNameLength = $("<h2>");
      //set alert to make sure teh name is at least 3 char length
      alertNameLength
        .text("Please enter a name at least 3 characters long.")
        .css("color", "red")
        .addClass("error-txt");
...
function chooseQuestion() {
  ...
  if (!usedQuestions.includes(currentQuestion)) {
    ...
    if (answerEl.children().length == 3) {
      //if this is the first question
      showQuestion();
    }
  } else if (usedQuestions.length == myQuestions.length) {
    //if all questions have been used then exit
    secondsLeft = 1;
...
function showAnswers() {
  answerA
    .text("A: " + myQuestions[currentQuestion].answers.a)
    .on("click", function () {
      chosenAnswer = "a";
      nextQuestion();
    });
  ...
  answerB
    .text("B: " + myQuestions[currentQuestion].answers.b)
    .on("click", function () {
      chosenAnswer = "b";
      nextQuestion();
    });
  ...
  answerC
    .text("C: " + myQuestions[currentQuestion].answers.c)
    .on("click", function () {
      chosenAnswer = "c";
      nextQuestion();
    });
  ...
  answerD
    .text("D: " + myQuestions[currentQuestion].answers.d)
    .on("click", function () {
      chosenAnswer = "d";
      nextQuestion();
    });
...
function showFinalPage() {
  ...
  var finalPercentage = $("<h2>").attr({
    class: "finalPage label",
    id: "finalPercentage",
  });
  ...
  lblFinalScore.text(
    "Congratulations " + nameInput.val() + " on your score of:  "
  );
